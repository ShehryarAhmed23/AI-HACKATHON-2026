...
function formatKilobytes(bytes = 0) {
...
function requestCard(request, users) {
  ...
  const attachmentTag = request.attachment?.name ? `<span class="tag">Attachment</span>` : "";
  return `
...
function renderCreateRequest() {
  ...
  const attachmentInput = qs("#request-attachment");
  ...
  qs("[data-create-form]").addEventListener("submit", async (event) => {
    event.preventDefault();
    const file = attachmentInput.files?.[0] || null;
    if (file && file.size > 5 * 1024 * 1024) {
      showToast("Attachment must be 5MB or less.");
      return;
    }
    ...
    try {
      ...
      showToast("Could not read selected file. Try another file.");
      return;
    }
...
function renderRequestDetail() {
  ...
  if (request.attachment?.name) {
    const isImage = request.attachment.type.startsWith("image/");
    qs("[data-request-attachment]").innerHTML = `
      ${isImage ? `<a href="${request.attachment.dataUrl}" target="_blank" rel="noreferrer"><img class="attachment-preview" src="${request.attachment.dataUrl}" alt="${request.attachment.name}"/></a>` : ""}
      <a class="btn btn-secondary" href="${request.attachment.dataUrl}" download="${request.attachment.name}">Download ${request.attachment.name}</a>
      <p>${formatKilobytes(request.attachment.size)}</p>
    `;
  } else {
    qs("[data-request-attachment]").innerHTML = `<p class="muted">No file attached for this request.</p>`;
  }
